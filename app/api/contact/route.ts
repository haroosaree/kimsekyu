...
export async function POST(request: Request) {
  ...
  const smtpUser = process.env.GMAIL_SMTP_USER || process.env.EMAIL_USER;
  const smtpPassword = process.env.GMAIL_SMTP_PASSWORD || process.env.EMAIL_PASSWORD;
  const emailFrom = process.env.EMAIL_FROM || smtpUser;
  if (!smtpUser || !smtpPassword || !emailFrom) return NextResponse.json({ message: "이메일 서비스가 아직 설정되지 않았습니다." }, { status: 503 });
  ...
  try {
    const transporter = nodemailer.createTransport({ host: process.env.EMAIL_HOST || "smtp.gmail.com", port: Number(process.env.EMAIL_PORT || 465), secure: process.env.EMAIL_SECURE !== "false", auth: { user: smtpUser, pass: smtpPassword } });
    await transporter.sendMail({ from: { address: emailFrom, name: "Kim Sekyu Real Estate" }, to: recipient, replyTo: email, subject: `[문의하기] ${subject}`, html: `<h2>${safeSubject}</h2><p><strong>이름:</strong> ${safeName}<br><strong>이메일:</strong> ${safeEmail}<br><strong>전화번호:</strong> ${safePhone}</p><p>${safeMessage}</p>`, text: `제목: ${subject}\n이름: ${name}\n이메일: ${email}\n전화번호: ${phone || "미입력"}\n\n${message}` });
  } catch (error) {
    console.error("Gmail SMTP rejected contact message", error);
