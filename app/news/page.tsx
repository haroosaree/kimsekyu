...
export default async function NewsIndex({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const initialItems = news.docs.map((item) => ({ id: item.id, title: item.title as string, slug: item.slug, publishedAt: item.publishedAt, viewCount: (item.legacyViewCount ?? 0) + (item.readCount ?? 0), thumbnailURL: legacyThumbnailURL(item.contentHTML as string) }));
