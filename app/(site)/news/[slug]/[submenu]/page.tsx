...
export default async function NewsSubmenu({ params, searchParams }: { params: Promise<{ slug: string; submenu: string }>; searchParams: Promise<{ page?: string }> }) {
  ...
  return <><MenuHero menuHref={`/${slug}/${submenu}`} /><main className="archive-page">
    <nav className="breadcrumb" aria-label="현재 위치"><Link href="/">홈</Link><span>›</span><Link href={isResource ? "/resources" : "/austin-economy"}>{isResource ? "자료실" : "어스틴 경제/뉴스"}</Link><span>›</span><span>{archive.title}</span></nav>
