...
export default async function NewsIndex({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const news = await payload.find({ collection: "news-feed", depth: 0, limit: PAGE_SIZE, page, sort: "-publishedAt", overrideAccess: true });
  const initialItems = news.docs.map((item) => ({ id: item.id, title: item.title as string, slug: item.slug, publishedAt: item.publishedAt, viewCount: item.viewCount ?? 0, thumbnailURL: legacyThumbnailURL(item.legacyContent as string) }));
