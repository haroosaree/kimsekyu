...
export default async function QuestionsPage({ searchParams }: { searchParams: Promise<{ ask?: string; page?: string }> }) {
  ...
  const questions = await payload.find({ collection: "questions", where: { subject: { not_like: "Re:%" } }, depth: 0, limit: PAGE_SIZE, page, sort: "-createdAt", overrideAccess: true, select: { subject: true, message: true, name: true, createdAt: true, publishedAt: true, viewCount: true } });
  ...
  return <><MenuHero /><main className="article-page">
    ...
    <p className="question-intro">부동산과 어스틴 생활에 관한 질문을 확인하세요. 답변과 연락처는 관리자만 확인할 수 있습니다.</p>
    {ask === "1" && <QuestionForm />}
